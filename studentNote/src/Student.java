public class Student {
    ...
    void addBulkExamsNote(int sNote1, int sNote2, int sNote3){
    ...
    void isPass(){
        this.avarage = (((this.c1.note * 0.80) + (this.c1.sNote * 0.20))+ ((this.c2.note * 0.80) + (this.c2.sNote * 0.20))
                + ((this.c3.note * 0.80) + (this.c3.sNote * 0.20))) / 3.0;
        System.out.println("---------------");
        if (avarage >= 55){
            System.out.println("Tebrikler sınıfı geçtin.");
            this.isPass = true;
        }else {
            System.out.println("Üzgünüz sınıfta kaldın.");
            this.isPass = false;
        }
        printNote();
    }
